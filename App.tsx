...
const App: React.FC = () => {
  ...
  const startStory = async () => {
    ...
    setIsAiLoading(true);
    try {
      // process.env.API_KEY가 빌드 시점에 비어있을 수 있으므로 런타임 체크
      ...
      const response = await ai.models.generateContent({
        ...
        contents: "신규 세션을 시작하라. 팍한의 집무실에서 주인공을 맞이하는 첫 장면을 묘사하라. 차가운 공기와 보드카 향기, 팍한의 위압감을 강조하라.",
        config: {
      ...
      setMessages([{ 
        role: 'assistant', 
        content: response.text || "침묵이 흐릅니다.", 
      ...
      if (e.message === "API_KEY_MISSING") {
        errorMsg = "API 키가 설정되지 않았습니다. Cloudflare 설정이나 AI Studio 키 선택을 확인해주세요.";
      } else if (e.message?.includes("not found")) {
        const aiStudio = (window as any).aistudio;
        if (aiStudio) await aiStudio.openSelectKey();
        errorMsg = "API 키를 다시 선택해주세요.";
      }
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 bg-black flex flex-col justify-center items-center z-[100]">
        <div className="font-noir text-3xl text-white mb-8 tracking-[0.3em] uppercase font-bold text-center px-4">
        ...
        {!showEnter ? (
          ...
          <button onClick={handleEnter} className="border border-red-900 bg-black text-red-600 px-10 py-3 font-noir tracking-[0.3em] text-xs uppercase hover:bg-red-900 hover:text-white transition-all shadow-[0_0_15px_rgba(139,0,0,0.3)] animate-pulse">
            [ ENTER ARCHIVES ]
          </button>
        )}
      </div>
        ...
        <div className="max-w-md w-full border border-neutral-800 bg-black/40 backdrop-blur-md p-8 rounded-sm border-l-4 border-l-red-900 shadow-2xl">
          ...
          <form onSubmit={(e) => { e.preventDefault(); setStep('dashboard'); }} className="space-y-6">
            <input 
              ...
              placeholder="NAME" value={profile.name} onChange={e => setProfile({...profile, name: e.target.value})} required
            />
            ...
            <textarea className="w-full bg-neutral-900/50 border border-neutral-800 p-3 text-sm focus:border-red-900 outline-none h-24 font-typewriter text-white" placeholder="APPEARANCE & VIBE" value={profile.appearance} onChange={e => setProfile({...profile, appearance: e.target.value})} />
            <button className="w-full bg-red-950 text-white font-noir py-4 text-xs tracking-widest hover:bg-red-800 transition-all uppercase shadow-lg">Confirm Identity</button>
          </form>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen p-8 max-w-7xl mx-auto">
        <header className="mb-16 border-b border-neutral-800 pb-8 flex justify-between items-end">
          ...
          <button onClick={resetApp} className="text-[10px] font-mono text-neutral-600 hover:text-red-600 transition-colors uppercase tracking-tighter">[ RESET_PROFILE ]</button>
        </header>
        ...
        <div className="max-w-2xl mx-auto text-center border border-neutral-800 p-12 bg-neutral-900/20 backdrop-blur-sm relative overflow-hidden">
          ...
          <p className="font-script text-xl text-neutral-400 mb-8 leading-relaxed">"늑대는 울지 않는다. 단지 사냥할 뿐."</p>
          <button onClick={startStory} className="px-16 py-4 bg-white text-black font-noir font-bold text-xs tracking-widest hover:bg-neutral-300 transition-all uppercase shadow-[0_0_30px_rgba(255,255,255,0.1)]">
            Deploy Session
          </button>
        </div>
      </div>
    ...
    <div className="h-screen flex flex-col bg-[#050505]">
      <header className="p-4 border-b border-neutral-900 flex justify-between items-center bg-black/80 z-20 backdrop-blur-md">
        <div className="flex items-center gap-4">
          ...
          <button onClick={resetApp} className="text-[9px] font-mono text-neutral-600 hover:text-red-800 transition-colors uppercase ml-4">[ TERMINATE ]</button>
        </div>
      </header>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-8 max-w-4xl mx-auto w-full z-10 scroll-smooth">
        ...
        {isAiLoading && <div className="flex gap-2 p-2"><div className="w-1.5 h-1.5 bg-red-900 rounded-full animate-bounce"></div></div>}
      </div>
      ...
      <div className="p-6 bg-black border-t border-neutral-900 z-20">
        <form onSubmit={sendMessage} className="max-w-4xl mx-auto flex gap-4">
          <input className="flex-1 bg-neutral-900 border border-neutral-800 p-4 text-sm text-white outline-none font-typewriter" placeholder="TYPE YOUR RESPONSE..." value={userInput} onChange={e => setUserInput(e.target.value)} disabled={isAiLoading} />
          <button className="px-8 bg-red-950 text-white font-noir text-xs tracking-widest disabled:opacity-50 transition-all uppercase" disabled={isAiLoading}>EXECUTE</button>
        </form>
      </div>
    </div>
  );
